...
"use strict";angular.module("core").config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise("/"),$stateProvider.state("home",{url:"/",templateUrl:"modules/core/views/home.client.view.html",controller:"HomeCtrl"}).state("learn",{url:"/learn",templateUrl:"modules/core/views/learn.client.view.html",controller:"LearnCtrl"}).state("planning",{url:"/planning",templateUrl:"modules/core/views/planning.client.view.html",controller:"PlanningCtrl"}).state("history",{url:"/history",templateUrl:"modules/core/views/history.client.view.html",controller:"HistoryCtrl"}).state("electricity",{url:"/electricity",templateUrl:"modules/core/views/electricity.client.view.html",controller:"ElectricityCtrl"})}]);
"use strict";angular.module("core").controller("ElectricityCtrl",["$scope","$http",function($scope,$http){$scope.date=moment(),$scope.dateOptions=[moment(),moment().subtract(1,"days"),moment().subtract(2,"days"),moment().subtract(3,"days")],$scope.$watch("date",function(newValue){var pricingDate,tempData=[[],[]];pricingDate="object"==typeof newValue?newValue.format("YYYYMMDD"):newValue.substring(1,11).replace(/\D/g,""),$http({url:"/cors-proxy",method:"GET",params:{host:"https://rrtp.comed.com/rrtp/ServletFeed",type:"daynexttoday",date:pricingDate}}).then(function(payload){var payloadArray=payload.data.replace(/[\[\]]/g,"").split(", ");return payloadArray.forEach(function(item,index){index%2!==0&&tempData[0].push(parseFloat(item))}),$http({url:"/cors-proxy",method:"GET",params:{host:"https://rrtp.comed.com/rrtp/ServletFeed",type:"day",date:pricingDate}})},function(){console.log("Error retreiving data!")}).then(function(payload){var payloadArray=payload.data.replace(/[\[\]]/g,"").split(", "),payloadCount=0;payloadArray.forEach(function(item,index){index%2!==0&&(tempData[1].push(parseFloat(item)),payloadCount++)});for(var i=0;24-payloadCount>i;i++)tempData[1].push(null);$scope.chartData=tempData},function(){console.log("Error retreiving data!")})}),$scope.labels=[$scope.date.format("MMM DD"),"","2am","","4am","","6am","","8am","","10am","","12pm","","2pm","","4pm","","6pm","","8pm","","10pm",""],$scope.series=["Day-Ahead Hourly Price","Real-Time Hourly Price"]}]);
"use strict";angular.module("core").controller("HeaderCtrl",["$mdToast","$location","$http","$mdDialog","$scope","$state","Authentication",function($mdToast,$location,$http,$mdDialog,$scope,$state,Authentication){$scope.state=$state,$scope.authentication=Authentication,$scope.$watch("state.current.name",function(){switch($state.current.name){case"home":$scope.tabIndex=0;break;case"planning":$scope.tabIndex=0;break;case"history":$scope.tabIndex=1;break;case"electricity":$scope.tabIndex=2;break;default:$scope.tabIndex=0}}),$scope.tabs=[{name:"Charge Planning",state:"planning",authenticationRequired:!0},{name:"Vehicle History",state:"history",authenticationRequired:!0},{name:"Electricity Pricing",state:"electricity"}],$scope.register=function(event){$mdDialog.show({targetEvent:event,templateUrl:"modules/users/dialogs/views/register.dialog.client.view.html",controller:"RegisterDialogCtrl"}).then(function(){$location.path("/planning"),$mdToast.show($mdToast.simple().content("Successfully Registered!").position("bottom right"))})},$scope.signIn=function(event){$mdDialog.show({targetEvent:event,templateUrl:"modules/users/dialogs/views/sign_in.dialog.client.view.html",controller:"SignInDialogCtrl"}).then(function(){$location.path("/planning"),$mdToast.show($mdToast.simple().content("Successfully Signed In!").position("bottom right"))})},$scope.signOut=function(){$http.get("/api/auth/signout").success(function(){$scope.authentication.user=null,$mdToast.show($mdToast.simple().content("Successfully Signed Out!").position("bottom right")),$location.path("/")}).error(function(response){$scope.error=response.message})}}]);
"use strict";angular.module("core").controller("HistoryCtrl",["$mdSidenav","$scope","Authentication",function($mdSidenav,$scope,Authentication){$scope.authentication=Authentication,$scope.year=2015,$scope.yearOptions=[2014,2015,2016],$scope.month="December",$scope.monthOptions=["ALL","January","February","March","April","May","June","July","August","September","October","November","December"],$scope.entries=[{month:"December",day:29,year:2015,target:"85",time:"8:00AM"},{month:"December",day:30,year:2015,target:"85",time:"8:00AM"},{month:"December",day:31,year:2015,target:"65",time:"1:30PM"},{month:"January",day:1,year:2016,target:"85",time:"9:00AM"},{month:"January",day:2,year:2016,date:"January 2, 2016",target:"100",time:"8:00AM"}]}]);
"use strict";angular.module("core").controller("HomeCtrl",["$mdDialog","$state","$scope","Authentication",function($mdDialog,$state,$scope,Authentication){$scope.authentication=Authentication,$scope.homeRegister=function(){$scope.authentication.user?$state.go("planning"):$mdDialog.show({targetEvent:event,templateUrl:"modules/users/dialogs/views/register.dialog.client.view.html",controller:"RegisterDialogCtrl"})},$scope.homeSignIn=function(){$scope.authentication.user?$state.go("planning"):$mdDialog.show({targetEvent:event,templateUrl:"modules/users/dialogs/views/sign_in.dialog.client.view.html",controller:"SignInDialogCtrl"})}}]);
"use strict";angular.module("core").controller("LearnCtrl",["$scope",function(){}]);
"use strict";angular.module("core").controller("PlanningCtrl",["$mdToast","Users","Authentication","$timeout","$scope","$mdDialog",function($mdToast,Users,Authentication,$timeout,$scope,$mdDialog){function equalSchedules(initial,current){var equality=!0;return initial.forEach(function(scheduleDay,index){(scheduleDay.active!==current[index].active||scheduleDay.time!==current[index].time||scheduleDay.target!==current[index].target)&&(equality=!1)}),equality}$scope.authentication=Authentication;var initialSchedule=$scope.authentication.user.schedule;$scope.chargeSettings=[{day:"Monday",active:!1,time:"8:00AM",target:38},{day:"Tuesday",active:!1,time:"8:00AM",target:38},{day:"Wednesday",active:!1,time:"8:00AM",target:38},{day:"Thursday",active:!1,time:"8:00AM",target:38},{day:"Friday",active:!1,time:"8:00AM",target:38},{day:"Saturday",active:!1,time:"8:00AM",target:38},{day:"Sunday",active:!1,time:"8:00AM",target:38}],0===initialSchedule.length?$timeout(function(){$scope.chargeSettings.forEach(function(setting){setting.active=!0,setting.target=85}),initialSchedule=angular.copy($scope.chargeSettings)},0):$timeout(function(){$scope.chargeSettings.forEach(function(setting,index){setting.active=initialSchedule[index].active,setting.target=initialSchedule[index].target,setting.time=initialSchedule[index].time})},0),$timeout(function(){$scope.$watch("chargeSettings",function(newVal){$scope.scheduleDirty=!equalSchedules(initialSchedule,newVal)},!0)},0),$scope.chargeCircleClick=function(index,event){$mdDialog.show({templateUrl:"modules/core/dialogs/views/planning.dialog.client.view.html",targetEvent:event,controller:"PlanningDialogCtrl",clickOutsideToClose:!1,locals:{currentData:$scope.chargeSettings[index]}}).then(function(data){$scope.chargeSettings[index].time=data.time,$scope.chargeSettings[index].target=data.target})},$scope.updateSchedule=function(){$scope.authentication.user.schedule=$scope.chargeSettings;var user=new Users($scope.authentication.user);user.$update(function(response){$mdToast.show($mdToast.simple().content("Updated Schedule Saved!").position("bottom right")),$scope.authentication.user=response,initialSchedule=response.schedule,$scope.scheduleDirty=!1},function(response){$mdToast.show($mdToast.simple().content(response.data.message).position("bottom right"))})}}]);
...
"use strict";angular.module("users").controller("RegisterDialogCtrl",["$timeout","$scope","$mdDialog","$http","$location","Authentication",function($timeout,$scope,$mdDialog,$http,$location,Authentication){$scope.authentication=Authentication,$scope.dialogCancel=function(){$mdDialog.cancel()},$scope.dialogRegister=function(){$http.post("/api/auth/register",$scope.credentials).success(function(response){$scope.authentication.user=response,$mdDialog.hide()}).error(function(response){$scope.error=response.message})}}]);
"use strict";angular.module("users").controller("SignInDialogCtrl",["Authentication","$http","$scope","$location","$mdDialog",function(Authentication,$http,$scope,$location,$mdDialog){$scope.authentication=Authentication,$scope.dialogCancel=function(){$mdDialog.cancel()},$scope.dialogSignIn=function(){$http.post("/api/auth/signin",$scope.credentials).success(function(response){$scope.authentication.user=response,$mdDialog.hide()}).error(function(response){$scope.error=response.message})}}]);