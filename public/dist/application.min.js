...
"use strict";angular.module("core").controller("HomeCtrl",["$mdToast","$mdDialog","$state","$scope","Authentication",function($mdToast,$mdDialog,$state,$scope,Authentication){$scope.authentication=Authentication,$scope.homeRegister=function(event){$scope.authentication.user?$state.go("planning"):$mdDialog.show({targetEvent:event,templateUrl:"modules/users/dialogs/views/register.dialog.client.view.html",controller:"RegisterDialogCtrl"}).then(function(){$state.go("planning"),$mdToast.show($mdToast.simple().content("Successfully Registered!").position("bottom right"))})},$scope.homeSignIn=function(event){$scope.authentication.user?$state.go("planning"):$mdDialog.show({targetEvent:event,templateUrl:"modules/users/dialogs/views/sign_in.dialog.client.view.html",controller:"SignInDialogCtrl"}).then(function(){$state.go("planning"),$mdToast.show($mdToast.simple().content("Successfully Signed In!").position("bottom right"))})}}]);
...
"use strict";angular.module("vehicles").controller("PlanningCtrl",["$mdToast","Vehicles","Users","Authentication","$timeout","$scope","$mdDialog",function($mdToast,Vehicles,Users,Authentication,$timeout,$scope,$mdDialog){function equalSchedules(initial,current){var equality=!0;return initial.forEach(function(scheduleDay,index){(scheduleDay.active!==current[index].active||scheduleDay.time!==current[index].time||scheduleDay.target!==current[index].target)&&(equality=!1)}),equality}$scope.authentication=Authentication;var initialSchedule=Authentication.user.vehicles[0].schedule;$scope.chargeSettings=[{day:"Monday",active:!1,time:"8:00AM",target:38},{day:"Tuesday",active:!1,time:"8:00AM",target:38},{day:"Wednesday",active:!1,time:"8:00AM",target:38},{day:"Thursday",active:!1,time:"8:00AM",target:38},{day:"Friday",active:!1,time:"8:00AM",target:38},{day:"Saturday",active:!1,time:"8:00AM",target:38},{day:"Sunday",active:!1,time:"8:00AM",target:38}],0===initialSchedule.length?$timeout(function(){$scope.chargeSettings.forEach(function(setting){setting.active=!0,setting.target=85}),initialSchedule=angular.copy($scope.chargeSettings)},0):$timeout(function(){$scope.chargeSettings.forEach(function(setting,index){setting.active=initialSchedule[index].active,setting.target=initialSchedule[index].target,setting.time=initialSchedule[index].time})},0),$timeout(function(){$scope.$watch("chargeSettings",function(newVal){$scope.scheduleDirty=!equalSchedules(initialSchedule,newVal)},!0)},0),$scope.chargeCircleClick=function(index,event){$mdDialog.show({templateUrl:"modules/vehicles/dialogs/views/planning.dialog.client.view.html",targetEvent:event,controller:"PlanningDialogCtrl",clickOutsideToClose:!1,locals:{currentData:$scope.chargeSettings[index]}}).then(function(data){$scope.chargeSettings[index].time=data.time,$scope.chargeSettings[index].target=data.target})},$scope.updateSchedule=function(){var tempVehicle=Authentication.user.vehicles[0];tempVehicle.schedule=$scope.chargeSettings;var vehicle=new Vehicles(tempVehicle);vehicle.$update(function(payload){$mdToast.show($mdToast.simple().content("Updated Schedule Saved!").position("bottom right")),Authentication.user.vehicles[0]=payload,initialSchedule=payload.schedule,$scope.scheduleDirty=!1},function(error){$mdToast.show($mdToast.simple().content("Unable to save: "+(error.data.message||"Database error")).position("bottom right"))})}}]);
...
"use strict";angular.module("users").controller("RegisterDialogCtrl",["Users","Vehicles","$timeout","$scope","$mdDialog","$http","$location","Authentication",function(Users,Vehicles,$timeout,$scope,$mdDialog,$http,$location,Authentication){$scope.authentication=Authentication,$scope.dialogCancel=function(){$mdDialog.cancel()},$scope.dialogRegister=function(){$http.post("/api/auth/register",$scope.credentials).then(function(payload){Authentication.user=payload.data;var newVehicle=new Vehicles({user:Authentication.user.id,manufacturer:"Tesla",model:"Model S"});return newVehicle.$save()}).then(function(payload){var tempUser=Authentication.user;-1===tempUser.vehicles.indexOf(payload._id)&&tempUser.vehicles.push(payload._id);var user=new Users(tempUser);return user.$update()}).then(function(payload){Authentication.user=payload,$mdDialog.hide()}).catch(function(error){$scope.error=error.data.message})}}]);
"use strict";angular.module("users").controller("SignInDialogCtrl",["Authentication","$http","$scope","$location","$mdDialog",function(Authentication,$http,$scope,$location,$mdDialog){$scope.authentication=Authentication,$scope.dialogCancel=function(){$mdDialog.cancel()},$scope.dialogSignIn=function(){$http.post("/api/auth/signin",$scope.credentials).success(function(response){$scope.authentication.user=response,$scope.authentication.user.currentVehicle=response.vehicles[0],$mdDialog.hide()}).error(function(response){$scope.error=response.message})}}]);
"use strict";angular.module("vehicles").controller("PlanningDialogCtrl",["currentData","$scope","$mdDialog",function(currentData,$scope,$mdDialog){$scope.day=currentData.day;var time=currentData.time,timeLength=time.length;$scope.hours=7===timeLength?time.substring(0,2):"0"+time.substring(0,1),$scope.hourOpts=["01","02","03","04","05","06","07","08","09","10","11","12"],$scope.minutes=time.substring(timeLength-4,timeLength-2),$scope.minuteOpts=["00","05","10","15","20","25","30","35","40","45","50","55"],$scope.meridian=time.substring(timeLength-2,timeLength),$scope.meridianOpts=["AM","PM"],$scope.target=currentData.target,$scope.dialogCancel=function(){$mdDialog.cancel()},$scope.dialogConfirm=function(){var data={time:("0"===$scope.hours.substring(0,1)?$scope.hours.substring(1,2):$scope.hours)+":"+$scope.minutes+$scope.meridian,target:$scope.target};$mdDialog.hide(data)}}]);