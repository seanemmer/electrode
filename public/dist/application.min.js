...
"use strict";angular.module("users").controller("SignInDialogCtrl",["Authentication","$http","$scope","$location","$mdDialog",function(Authentication,$http,$scope,$location,$mdDialog){$scope.authentication=Authentication,$scope.dialogCancel=function(){$mdDialog.cancel()},$scope.dialogSignIn=function(){$http.post("/api/auth/signin",$scope.credentials).success(function(response){console.log("signin"),console.log(response),$scope.authentication.user=response,$scope.authentication.user.currentVehicle=response.vehicles[0],$mdDialog.hide()}).error(function(response){$scope.error=response.message})}}]);
